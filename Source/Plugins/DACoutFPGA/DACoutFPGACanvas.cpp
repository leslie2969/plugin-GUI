...
DACoutFPGACanvas::DACoutFPGACanvas(DACoutFPGA* n) :
    processor(n)
{
    ...
    int yOffset = 50;
    processor = n;
    display = new DACoutFPGADisplay(this, viewport, n);
...
DACoutFPGACanvas::~DACoutFPGACanvas()
{
    //delete scale;
    //deleteAllChildren();
}

void DACoutFPGACanvas::beginAnimation()
{
...
void DACoutFPGACanvas::endAnimation()
{
...
void DACoutFPGACanvas::update()
{
    repaint();
}


void DACoutFPGACanvas::refreshState()
{
    resized();
}

void DACoutFPGACanvas::resized()
{
...
void DACoutFPGACanvas::paint(Graphics& g)
{
...
void DACoutFPGACanvas::repaintDisplay(){
    display->repaint();
}

void DACoutFPGACanvas::refresh()
{
    // called every 10 Hz
...
bool DACoutFPGACanvas::keyPressed(const KeyPress& key)
{
    return false;
}

void DACoutFPGACanvas::buttonClicked(Button* button)
{
...
void DACoutFPGACanvas::setBin(int bin_){
    bin = bin_;
}

void DACoutFPGACanvas::setBinSize(int binSize_){
    binSize = binSize_;
}

void DACoutFPGACanvas::setData(double data_){
    data=data_;
}
...
DACoutFPGADisplay::DACoutFPGADisplay(DACoutFPGACanvas* c, Viewport* v, DACoutFPGA* p){
    processor=p;
    canvas=c;
    viewport=v;
...
DACoutFPGADisplay::~DACoutFPGADisplay(){
    deleteAllChildren();
}

void DACoutFPGADisplay::visibleAreaChanged(const Rectangle<int>& newVisibleArea){
    
}

void DACoutFPGADisplay::viewedComponentChanged(Component* newComponent){
    
}

void DACoutFPGADisplay::resized()
{
    int width = getWidth();
...
void DACoutFPGADisplay::paint(Graphics &g)
{
    ...
    for (int i = 0 ; i < histoData.size() ; i++){
		...
		graph = new GraphUnit(processor, canvas, channelColours[(uint64(minMaxMean[i][0]) + sizeof(channelColours)) % (sizeof(channelColours))], "a", &minMaxMean[i][1], histoData[i]); // pass &histoData[i][2] instead of 3 to pass on how many bins are used
...
void DACoutFPGADisplay::refresh()
{
    for (int i = 0 ; i < graphs.size() ; i++){
        graphs[i]->repaint();
    }
...
int DACoutFPGADisplay::getNumGraphs()
{
    return graphs.size();
}
...
GraphUnit::GraphUnit(DACoutFPGA* processor_, DACoutFPGACanvas* canvas_, juce::Colour color_, String name_, float  * stats_, std::vector<double> data_){
...
HistoGraph::HistoGraph(DACoutFPGA* processor_, DACoutFPGACanvas* canvas_, juce::Colour color_, uint64 bins_, float max_, std::vector<double> histoData_)
{
    color = color_;
...
StatDisplay::StatDisplay(DACoutFPGA* processor_, juce::Colour c, float * s)
{
    processor=processor_;
    color = c;
    stats = s;
